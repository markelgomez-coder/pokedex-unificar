...
const numeroPokemon = new URLSearchParams(window.location.search).get("pokemon");
if (numeroPokemon != null)
    funcionesAPI.obtenerPokemon(numeroPokemon).then(async (pokemon) => {
        ...
        const descripcion = await funcionesAPI.obtenerPokemonDescripcion(numeroPokemon);
        ...
        const evolucionesLink = await funcionesAPI.obtenerPokemonEvolucionesLink(numeroPokemon);
        const evolucion = await funcionesAPI.obtenerPokemonEvoluciones(evolucionesLink);
        const evoluciones = await Promise.all(evolucion.map((evo) => funcionesAPI.obtenerPokemon(evo.nombre)));
        if (container != null) {
...
async function mostrarPanelDerecha(descripcion, dobleDano, mitadDano, noDano, evoluciones) {
    const dobleDanoHTML = dobleDano
        .map((d) => `<span class="dano ${d.name}">${d.name.charAt(0).toUpperCase() + d.name.slice(1)}</span>`)
        .join("");
...
if (window.location.pathname.endsWith("panel-pokemon.html")) {
    document.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("icono-dream-team-interior") ||
            target.classList.contains("icono-dream-team-vector1") ||
            target.classList.contains("icono-dream-team-vector2")) {
            const card = target.closest(".carta-pokemon");
            if (card) {
                const nombrePokemon = card.querySelector(".pokemon-name");
                if (nombrePokemon) {
                    const nombrePokemonMinusculas = nombrePokemon.textContent.toLowerCase();
                    const icono = card.getElementsByClassName("icono-dream-team-vector2")[0];
                    funcionesDreamTeam.modificarPokemonDreamTeamDesdeCarta(nombrePokemonMinusculas, icono);
                }
            }
        }
